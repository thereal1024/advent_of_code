...
def solution(lines):
    ...
    def contains(bag_type, target):
        items = rules[bag_type]
        return any(contained_type == target or contains(contained_type, target) for _, contained_type in items)
    
    total_count = sum(contains(bag_type, 'shiny gold') for bag_type in rules)
