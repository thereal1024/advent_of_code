...
def most_geo(bp, time):
    ...
    def geo_rec(bots, mats, time):
        nonlocal bp, max_items, search_best
        
        geos = mats[3]
        if time == 0:
            search_best = max(search_best, geos)
            return geos
        
        geobots = bots[3]
        # if it is already not possible to beat global search, bail this path
        if geos + (time * (2 * geobots + time - 1) // 2) <= search_best:
            return -1
...
def solution(lines):
    ...
    geos = [most_geo(bp, TIME) for bp in tqdm(blueprints)]
    soln = math.prod(geos)
